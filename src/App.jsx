        ...
        {adminSub === "rooster" && (() => {
          ...
          function buildRoosterVolgorde(wns) {
            ...
            const VASTE_VOLGORDE = [
              ...
              "Erwin Kooistra",          // 9
              "Sjoerd Kaper",            // 10
              "Alwin Speelman",          // 11  → daarna stagiair Chemie
              "Ina Betlehem Stevens",    // 13  → daarna stagiair Orglab
              "Nadia Keuning",           // 15
              "Melanie Schep",           // 17  → daarna stagiair Micro
              "Suleika Ramdien",         // 18
              "Dylan Breebaard",         // 19
              "Diana Wijbenga",          // 20
              "Xamira De Haan",          // 21
            ...
            // Sorteer vaste werknemers op naam-volgorde
            const vast = wns.filter(w => !w.stagiair).sort((a, b) => {
              const ai = VASTE_VOLGORDE.indexOf(a.naam);
              const bi = VASTE_VOLGORDE.indexOf(b.naam);
              if (ai === -1 && bi === -1) return (a.roosterVolgorde||99) - (b.roosterVolgorde||99);
              if (ai === -1) return 1;
              if (bi === -1) return -1;
              return ai - bi;
            });
