...
function WerknemerPortaal({ werknemer, aanvragen, werknemers, onNieuweAanvraag, onTerug, onUpdateStagiair }) {
      ...
      <div style={{ background: "linear-gradient(90deg, #1A2C42, #1E3A52)", borderBottom: "1px solid rgba(255,255,255,0.07)", padding: "0 32px", display: "flex", alignItems: "center", justifyContent: "space-between", height: "64px", position: "sticky", top: 0, zIndex: 100, boxShadow: "0 4px 24px rgba(0,0,0,0.3)" }}>
        ...
        <nav style={{ display: "flex", gap: "4px" }}>
          {(() => {
              ...
              return [{ key: "aanvragen", label: "📋 Aanvragen" }, { key: "werknemers", label: "👥 Werknemers" }, { key: "stagiairs", label: "🎓 Stagiairs" }, { key: "kalender", label: "📅 Overzicht" }, { key: "rooster", label: "🗓️ Rooster" }, { key: "jaar", label: "📊 Jaaroverzicht" }].map(item => (
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "32px 24px" }}>
                ...
                return (
                  <div key={aanvraag.id} style={{ background: "linear-gradient(135deg, #1A2C42, #162233)", borderRadius: "12px", padding: "20px", border: problem ? "1px solid rgba(224,85,85,0.4)" : "1px solid rgba(255,255,255,0.06)", boxShadow: problem ? "0 0 16px rgba(224,85,85,0.1)" : "0 4px 16px rgba(0,0,0,0.2)", display: "flex", alignItems: "center", gap: "16px" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "10px", flexShrink: 0 }}>
                      ...
                      <button onClick={() => setConfirmModal({ vraag: "Aanvraag verwijderen?", onJa: () => setAanvragen(prev => prev.filter(a => a.id !== aanvraag.id)) })} style={{ padding: "6px 10px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.04)", color: "#4A6A82", cursor: "pointer", fontSize: "13px" }} title="Verwijderen">🗑</button>
                    </div>
                  </div>
                );
              })}
        ...
        {adminSub === "rooster" && (() => {
          ...
          function buildRoosterVolgorde(wns) {
            ...
            // Vaste volgorde op roosterVolgorde, stagiairs eruit filteren
            const vast = wns.filter(w => !w.stagiair).sort((a,b) => (a.roosterVolgorde||99) - (b.roosterVolgorde||99));
            // Bouw de uiteindelijke lijst met stagiairs op de juiste plek
            // Chemie stagiair na Monique (pos 5), voor Anita (pos 8)
            // Orglab stagiair na Ina (pos 13), voor Nadia (pos 15)
            // Micro stagiair na Suleika (pos 18), voor Diana (pos 20)
            const result = [];
            for (const w of vast) {
              result.push(w);
              if (w.roosterVolgorde === 11) stagChemie.forEach(s => result.push(s)); // na Alwin
              if (w.roosterVolgorde === 13) stagOrglab.forEach(s => result.push(s)); // na Ina
              if (w.roosterVolgorde === 17) stagMicro.forEach(s => result.push(s));  // na Melanie
            }
            return result;
          }
          ...
          return (
            <div style={{ fontFamily:"Georgia, serif" }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:"14px", marginBottom:"14px", flexWrap:"wrap" }}>
                ...
                <button onClick={() => window.print()} style={{ marginLeft:"auto", background:"rgba(224,85,85,0.15)", border:"1px solid rgba(224,85,85,0.3)", color:"#E05555", borderRadius:"8px", padding:"6px 14px", cursor:"pointer", fontSize:"12px", fontFamily:"Georgia, serif", fontWeight:"bold" }}>&#128424; Afdrukken</button>
              </div>
                ...
                <table style={{ borderCollapse:"collapse", fontSize:"10px" }}>
                    ...
                    <tr>
                      ...
                      {gesorteerdeWerknemers.map(w => (
                        <th key={w.id} colSpan={2} style={{ background:"#0D1820", border:"1px solid rgba(255,255,255,0.08)", borderLeft:"2px solid rgba(255,255,255,0.35)", padding:"4px 3px", textAlign:"center", whiteSpace:"nowrap" }}>
                          <span style={{ color:"#C8D8E8", fontSize:"10px", fontWeight:"bold" }}>{w.naam.split(" ")[0]}</span>
                        </th>
                      ))}
                    </tr>
                    {/* O/M */}
                  ...
                  <tbody>
                    {werkdagen.map(({ datum, dagStr, dag, dow }) => {
                      ...
                      const isMa = dow === 1;
                      return (
                        <tr key={dagStr} style={{ borderTop: isMa ? "2px solid rgba(74,158,224,0.3)" : "none", borderBottom: "1px solid rgba(255,255,255,0.12)" }}>
                            ...
                            return ["O","M"].map(deel => {
                              ...
                              return (
                                <td key={w.id+deel}
                                  ...
                                  style={{ background: "transparent", border: afd && stagInOpleiding ? "2px solid #E67E22" : "1px solid rgba(255,255,255,0.05)", borderLeft: deel==="O" ? "2px solid rgba(255,255,255,0.35)" : "1px solid rgba(255,255,255,0.04)", padding:"2px", verticalAlign:"middle", textAlign:"center", height:"26px", position:"relative", cursor: afd ? "pointer" : "default" }}>
                                  {afd ? (
                                    <div style={{ width:"100%", height:"calc(100% - 0px)", display:"flex", alignItems:"center", justifyContent:"center", position:"relative", background: bg, borderRadius:"3px", overflow:"hidden" }}>
                                      <span style={{ fontSize: letter === "OPL" ? "8px" : "12px", fontWeight:"bold", color: verlof ? "rgba(255,255,255,0.5)" : "rgba(0,0,0,0.55)", textDecoration: verlof ? "line-through" : "none" }}>{letter}</span>
                                      {verlof && <span style={{ position:"absolute", top:"1px", right:"2px", fontSize:"8px" }}>🌴</span>}
                                    </div>
                                  ) : dienstreis ? (
                                    <div style={{ width:"100%", height:"100%", display:"flex", alignItems:"center", justifyContent:"center", background: bg, borderRadius:"3px" }}>
                                      <span style={{ fontSize:"8px", fontWeight:"bold", color:"rgba(0,0,0,0.55)" }}>DR</span>
                                    </div>
                                  ) : verlof ? (
                                    <div style={{ width:"100%", height:"100%", display:"flex", alignItems:"center", justifyContent:"center", background: bg, borderRadius:"3px" }}>
                    ...
                    return (
                      <div key={w.id} className="jaar-rij" style={{ display: "flex", alignItems: "center", gap: "0" }}>
                        ...
                        <div className="naam-kolom" style={{ width: "160px", flexShrink: 0, display: "flex", alignItems: "center", gap: "8px", paddingRight: "12px" }}>
                          ...
                          <span className="naam-label" style={{ fontSize: "12px", color: "#B0BEC8", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{w.naam.split(" ")[0]}</span>
                        </div>
