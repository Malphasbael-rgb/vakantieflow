      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "32px 24px" }}>
        ...
        {adminSub === "rooster" && (() => {
          ...
          function buildRoosterVolgorde(wns) {
            // Vaste naamvolgorde — onafhankelijk van Supabase roosterVolgorde
            const VASTE_VOLGORDE = [
              "Leon Segerink",
              "Elke Buikstra-bosklopper",
              "Roos Van Wijnen",
              "Nick Roskam",             // na Roos
              "Niels De Vries",
              "Monique Breijmer",
              ...
              "Erwin Kooistra",
              "Sjoerd Kaper",
              "Alwin Speelman",          // → daarna stagiair Chemie
              "Ina Betlehem Stevens",    // → daarna stagiair Orglab
              "Nadia Keuning",
              "Melanie Schep",           // → daarna stagiair Micro
              "Suleika Ramdien",
              "Diana Wijbenga",
              "Xamira De Haan",
              "Ylva",
            ...
            // ID → vaste positie mapping als extra zekerheid naast naam
            const ID_VOLGORDE = {
              9:0, 3:1, 13:2, 11:3, 16:4, 5:5, 1:6, 2:7, 6:8, 8:9,
              15:10, 4:11, 7:12, 10:13, 14:14, 17:15, 18:16,
              19:17, 20:18, 12:19, 21:20
            };
            const vast = wns.filter(w => !w.stagiair).sort((a, b) => {
              const ai = VASTE_VOLGORDE.indexOf(a.naam) !== -1 ? VASTE_VOLGORDE.indexOf(a.naam) : (ID_VOLGORDE[a.id] ?? 99);
              const bi = VASTE_VOLGORDE.indexOf(b.naam) !== -1 ? VASTE_VOLGORDE.indexOf(b.naam) : (ID_VOLGORDE[b.id] ?? 99);
              return ai - bi;
            });
            const result = [];
            for (const w of vast) {
              ...
              if (w.naam === "Nick Roskam")           stagBlik.forEach(s => result.push(s));
            }
