                  ...
                  return (
                    <div key={dagStr} style={{
                      ...
                      <div style={{ flex:1, display:"flex", gap:"10px", alignItems:"stretch" }}>
                        ...
                        {/* Rechts: audit banners (alleen als er iets is) */}
                        {(dagAudits.length > 0 || labBezoek.length > 0) && (
                          <div style={{ display:"flex", flexDirection:"column", gap:"4px", justifyContent:"center", minWidth:"140px", maxWidth:"180px" }}>
                            {dagAudits.map(a => (
                              <div key={a.id} style={{ background:"rgba(230,126,34,0.12)", border:"1px solid rgba(230,126,34,0.3)", borderRadius:"8px", padding:"5px 9px", display:"flex", alignItems:"center", gap:"6px" }}>
                                <span style={{ fontSize:"11px" }}>🔍</span>
                                <div>
                                  <div style={{ fontSize:"11px", fontWeight:"bold", color:"#E67E22", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis", maxWidth:"130px" }}>Audit: {a.bedrijf}</div>
                                  {a.opmerking && <div style={{ fontSize:"9px", color:"rgba(230,126,34,0.7)" }}>{a.opmerking}</div>}
                                </div>
                              </div>
                            ))}
                            {labBezoek.map(a => (
                              <div key={"lab-"+a.id} style={{ background:"rgba(74,158,224,0.12)", border:"1px solid rgba(74,158,224,0.35)", borderRadius:"8px", padding:"5px 9px", display:"flex", alignItems:"center", gap:"6px" }}>
                                <span style={{ fontSize:"11px" }}>🏭</span>
                                <div>
                                  <div style={{ fontSize:"11px", fontWeight:"bold", color:"#4A9EE0", whiteSpace:"nowrap" }}>Lab: {a.bedrijf}{a.labTijd ? ` ${a.labTijd}` : ""}</div>
                                  {a.opmerking && <div style={{ fontSize:"9px", color:"rgba(74,158,224,0.7)" }}>{a.opmerking}</div>}
                                </div>
                              </div>
                            ))}
                          </div>
