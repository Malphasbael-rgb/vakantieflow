...
function WerknemerPortaal({ werknemer, aanvragen, werknemers, onNieuweAanvraag, onTerug, onUpdateStagiair, onArchiveerAanvraag }) {
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0F1923 0%, #162233 60%, #0F1923 100%)", fontFamily: "Georgia, serif", color: "#E8EDF2" }}>
      ...
      <div style={{ maxWidth: "700px", margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ display: "flex", gap: "4px", marginBottom: "24px" }}>
          {[{ key: "aanvragen", label: "📋 Mijn aanvragen" }, { key: "archief", label: "📦 Archief" }, { key: "nieuw", label: "✏️ Nieuwe aanvraag" }].map(t => (
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0F1923 0%, #162233 50%, #0F1923 100%)", fontFamily: "Georgia, serif", color: "#E8EDF2" }}>
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "32px 24px" }}>
        ...
        {adminSub === "rooster" && (() => {
          ...
          function buildRoosterVolgorde(wns) {
            // Volgorde volledig op ID — onafhankelijk van naam of Supabase data
            // Leon=9, Elke=3, Roos=13, Nick=11, NielsV=16, Monique=5, Alinda=1,
            // Anita=2, Erwin=6, Sjoerd=8, Alwin=15, [Chemie stagiair],
            // Ina=4, [Orglab stagiair], Nadia=7, Melanie=10, [Micro stagiair],
            // Suleika=14, Diana=17, Xamira=18, Ylva=19, NielsR=20, Bart=12, Dylan=21, [Blik stagiair]
            const ID_POS = {
              9:0, 3:1, 13:2, 11:3, 16:4, 5:5, 1:6, 2:7, 6:8, 8:9,
              15:10, 4:12, 7:14, 10:15, 14:16,
              17:17, 18:18, 19:19, 20:20, 12:21, 21:22
            };
            ...
            const vast = wns.filter(w => !w.stagiair).sort((a, b) => (ID_POS[a.id] ?? 99) - (ID_POS[b.id] ?? 99));
            const result = [];
            for (const w of vast) {
              result.push(w);
              if (w.id === 15) stagChemie.forEach(s => result.push(s)); // na Alwin
              if (w.id === 4)  stagOrglab.forEach(s => result.push(s)); // na Ina
              if (w.id === 10) stagMicro.forEach(s => result.push(s));  // na Melanie
              if (w.id === 21) stagBlik.forEach(s => result.push(s));   // na Dylan
            }
            ...
            <div style={{ fontFamily:"Georgia, serif" }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:"14px", marginBottom:"14px", flexWrap:"wrap" }}>
                ...
                <button onClick={() => {
                    ...
                    const container = document.querySelector(".rooster-print-container");
                    const table = container ? container.querySelector("table") : null;
                    if (table) {
                      table.style.transform = "";
                      const pageW = 297; // A4 landscape mm → px bij 96dpi ≈ 1122px, maar we werken met mm
                      const tableW = table.scrollWidth;
                      const pageWpx = window.innerWidth;
                      const scale = Math.min(1, (pageWpx - 8) / tableW);
                    ...
                    setTimeout(() => {
                      ...
                      if (table) { table.style.transform = ""; table.style.display = ""; }
                    }, 2000);
      ...
      <style>{`
        ...
        @media print {
          ...
          body.rooster-print-active .rooster-print-container > div {
            overflow: visible !important;
