...
function isProblem(a, all, w) { return getConflicts(a, all, w).length > 0; }
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                  <div>
                    ...
                    <input type="date" value={form.van} onChange={e => setForm(f => ({ ...f, van: e.target.value }))} style={inputStyle} />
                  </div>
                  <div>
                    <label style={labelStyle}>Einddatum</label>
                    <input type="date" value={form.tot} onChange={e => setForm(f => ({ ...f, tot: e.target.value }))} style={inputStyle} />
                  </div>
                </div>
                {form.van && form.tot && (
                  <div style={{ background: "rgba(74,158,224,0.08)", border: "1px solid rgba(74,158,224,0.2)", borderRadius: "8px", padding: "10px 16px", fontSize: "13px", color: "#4A9EE0" }}>
                    📅 {dateDiff(form.van, form.tot)} dag{dateDiff(form.van, form.tot) > 1 ? "en" : ""}
          ...
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              {gefilterd.map(aanvraag => {
                ...
                const problem = conflicts.length > 0;
                  ...
                  <div key={aanvraag.id} style={{ background: "linear-gradient(135deg, #1A2C42, #162233)", borderRadius: "12px", padding: "20px", border: problem ? "1px solid rgba(224,85,85,0.4)" : "1px solid rgba(255,255,255,0.06)", boxShadow: problem ? "0 0 16px rgba(224,85,85,0.1)" : "0 4px 16px rgba(0,0,0,0.2)", display: "flex", alignItems: "center", gap: "16px" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px", flexWrap: "wrap" }}>
                        ...
                        {problem && <span style={{ fontSize: "11px", background: "rgba(224,85,85,0.15)", border: "1px solid rgba(224,85,85,0.3)", borderRadius: "6px", padding: "2px 8px", color: "#E05555" }}>⚠️ Conflict met {conflictNamen.join(", ")}</span>}
                      </div>
                      <div style={{ fontSize: "13px", color: "#7A9AB5" }}>📅 {aanvraag.van} → {aanvraag.tot} &nbsp;·&nbsp; {dateDiff(aanvraag.van, aanvraag.tot)} dag{dateDiff(aanvraag.van, aanvraag.tot) > 1 ? "en" : ""}{aanvraag.opmerking && <> &nbsp;·&nbsp; <span style={{ fontStyle: "italic" }}>{aanvraag.opmerking}</span></>}</div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "10px", flexShrink: 0 }}>
                      ...
                      {aanvraag.status === "in behandeling" && (
                        <div style={{ display: "flex", gap: "6px" }}>
                          <button onClick={() => handleStatusChange(aanvraag.id, "goedgekeurd")} style={{ padding: "6px 12px", borderRadius: "8px", border: "1px solid rgba(45,155,111,0.4)", background: "rgba(45,155,111,0.1)", color: "#2D9B6F", cursor: "pointer", fontSize: "12px", fontFamily: "Georgia, serif" }}>✓ Goed</button>
