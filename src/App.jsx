...
function WerknemerPortaal({ werknemer, aanvragen, werknemers, weekTemplate, onNieuweAanvraag, onTerug, onUpdateStagiair, onArchiveerAanvraag }) {
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0F1923 0%, #162233 60%, #0F1923 100%)", fontFamily: "Georgia, serif", color: "#E8EDF2" }}>
      ...
      <div style={{ maxWidth: "700px", margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ display: "flex", gap: "4px", marginBottom: "24px" }}>
          {[{ key: "aanvragen", label: "📋 Mijn aanvragen" }, { key: "archief", label: "📦 Archief" }, { key: "nieuw", label: "✏️ Nieuwe aanvraag" }, { key: "rooster", label: "📅 Mijn rooster" }].map(t => (
        ...
        {tab === "rooster" && (() => {
          ...
          const fmt = d => `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
          const fmtKort = d => `${d.getDate()} ${["jan","feb","mrt","apr","mei","jun","jul","aug","sep","okt","nov","dec"][d.getMonth()]}`;
          ...
          return (
            <div>
              ...
              {/* Debug info */}
              <div style={{ background:"rgba(255,0,0,0.1)", border:"1px solid red", borderRadius:"8px", padding:"10px", marginBottom:"12px", fontSize:"11px", color:"#E8EDF2", wordBreak:"break-all" }}>
                <div><b>Werknemer ID:</b> {werknemer.id} (type: {typeof werknemer.id})</div>
                <div><b>WeekTemplate datums:</b> {Object.keys(weekTemplate || {}).join(", ") || "leeg"}</div>
                <div><b>Eerste datum keys:</b> {Object.keys(weekTemplate || {}).length > 0 ? Object.keys(Object.values(weekTemplate)[0] || {}).slice(0,5).join(", ") : "–"}</div>
              </div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
                {werkdagen.map(dag => {
                  ...
                  const wIdStr = String(werknemer.id);
                  const ochtendVal = weekTemplate?.[dagStr]?.[wIdStr]?.["O"] ?? weekTemplate?.[dagStr]?.[werknemer.id]?.["O"] ?? null;
                  const middagVal  = weekTemplate?.[dagStr]?.[wIdStr]?.["M"] ?? weekTemplate?.[dagStr]?.[werknemer.id]?.["M"] ?? null;
                  const ochtend = typeof ochtendVal === "object" ? ochtendVal?.afd : ochtendVal;
                  const middag  = typeof middagVal  === "object" ? middagVal?.afd  : middagVal;
                  ...
                  return (
                    <div key={dagStr} style={{
                      ...
                      borderRadius:"12px",
                      border: isVandaag(dag) ? "1px solid rgba(74,158,224,0.4)" : isFeest ? "1px solid rgba(224,85,85,0.3)" : "1px solid rgba(255,255,255,0.05)",
                      padding:"14px 18px",
                      display:"flex",
                      alignItems:"center",
                      gap:"16px",
                      opacity: isFeest ? 0.7 : 1
                    }}>
                      {/* Datum kolom */}
                      <div style={{ minWidth:"60px", textAlign:"center" }}>
                        <div style={{ fontSize:"11px", color: isVandaag(dag) ? "#4A9EE0" : "#7A9AB5", textTransform:"uppercase", letterSpacing:"1px" }}>{dagNamen[dag.getDay()]}</div>
                        <div style={{ fontSize:"22px", fontWeight:"bold", color: isVandaag(dag) ? "#4A9EE0" : isFeest ? "#E05555" : "#E8EDF2", lineHeight:"1.2" }}>{dag.getDate()}</div>
                        <div style={{ fontSize:"10px", color:"#7A9AB5" }}>{["jan","feb","mrt","apr","mei","jun","jul","aug","sep","okt","nov","dec"][dag.getMonth()]}</div>
                      </div>

                      {/* Inhoud */}
                      <div style={{ flex:1 }}>
                        {isFeest && <div style={{ fontSize:"11px", color:"#E05555", marginBottom:"6px" }}>🇳🇱 {feestNaam}</div>}
                        ...
                        {!isFeest && !verlof && !dienstreis && (
                          <div style={{ display:"flex", gap:"10px" }}>
                            ...
                            <div style={{ flex:1, borderRadius:"8px", overflow:"hidden", border:"1px solid rgba(255,255,255,0.06)" }}>
                              <div style={{ background:"rgba(255,255,255,0.05)", padding:"4px 10px", fontSize:"10px", color:"#7A9AB5", letterSpacing:"1px", textTransform:"uppercase" }}>Ochtend</div>
                              <div style={{ padding:"8px 10px", background: ochtend ? kleurVoor(ochtend) : "rgba(255,255,255,0.02)", minHeight:"36px", display:"flex", alignItems:"center", justifyContent:"center" }}>
                                {ochtend
                                  ? <span style={{ fontSize:"14px", fontWeight:"bold", color:"rgba(0,0,0,0.75)" }}>{ochtend}</span>
                                  : <span style={{ fontSize:"12px", color:"rgba(255,255,255,0.2)", fontStyle:"italic" }}>–</span>}
                              </div>
                            </div>
                            {/* Middag */}
                            <div style={{ flex:1, borderRadius:"8px", overflow:"hidden", border:"1px solid rgba(255,255,255,0.06)" }}>
                              <div style={{ background:"rgba(255,255,255,0.05)", padding:"4px 10px", fontSize:"10px", color:"#7A9AB5", letterSpacing:"1px", textTransform:"uppercase" }}>Middag</div>
                              <div style={{ padding:"8px 10px", background: middag ? kleurVoor(middag) : "rgba(255,255,255,0.02)", minHeight:"36px", display:"flex", alignItems:"center", justifyContent:"center" }}>
                                {middag
                                  ? <span style={{ fontSize:"14px", fontWeight:"bold", color:"rgba(0,0,0,0.75)" }}>{middag}</span>
                                  : <span style={{ fontSize:"12px", color:"rgba(255,255,255,0.2)", fontStyle:"italic" }}>–</span>}
                              </div>
                            </div>
                          </div>
                        ...
                        {!isFeest && !verlof && !dienstreis && !ochtend && !middag && (
                          <div style={{ fontSize:"12px", color:"rgba(255,255,255,0.2)", fontStyle:"italic", marginTop:"4px" }}>Nog niet ingepland</div>
                        )}
                      </div>
